...
| `TODO.md` | 树状待办账本 | 每一项活到被勾掉 | **只改标记，不删行** |
| `STATE.md` | 进度、卡点、关键命令、下一步指向 `TODO.md` 的编号 | 到下次 `/clear` | 整个覆盖，≤150 行 |

混在一个文件里，决策层会被状态层的反复重写churn掉——你会发现自己在手动做 `HANDOFF_HISTORY_20260920.md` 这种归档。

### 为什么 todo 要单独一层

线性对话有个固有缺陷：旧消息列了 3 个 todo，做第一个时展开了 5 个子任务，做完子任务后剩下 2 个父项已经被滚出屏幕，
人和模型都忘了。把 todo 放在 `STATE.md` 里解决不了——`STATE.md` 每次整个覆盖，没被抄进去的项就丢了。

`TODO.md` 的规矩：树状嵌套（子任务缩进在被展开的父项下）；做完打 `[x]` 加日期，放弃打 `[-]` 写原因，**永远不删行**；
**聊天里列 todo 之前先写文件**，聊天里只展示文件的当前视图。`/todo` 随时对账，不必等 `/handoff`。
SessionStart 只注入未完成的子树，已完成的项及其子项被过滤掉，所以账本再长也不会撑大上下文。

## 用法

```
/todo           # 会话中随时对账：把聊天里出现过的 todo 合并进 TODO.md
/handoff        # 写入三个文件 + 只提交这三个文件
/clear          # 手动，见下
```
...
`STATE.md` 全量注入（有 150 行上限），`TODO.md` **只注入未完成子树**，`DECISIONS.md` **只注入标题 + 路径**，需要细节时模型自己 Read。
